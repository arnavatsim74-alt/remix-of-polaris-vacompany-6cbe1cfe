...
-- Replace the existing trigger function to also send Discord notifications on rank change
...
BEGIN
    IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
        ...
        -- If rank changed, send Discord notification
        IF old_rank IS DISTINCT FROM new_rank AND old_rank IS NOT NULL THEN
            ...
            IF supabase_url IS NOT NULL AND service_role_key IS NOT NULL THEN
                PERFORM extensions.http_post(
                    supabase_url || '/functions/v1/discord-rank-notification',
                    jsonb_build_object(
...
DROP TRIGGER IF EXISTS update_pilot_stats_trigger ON public.pireps;
