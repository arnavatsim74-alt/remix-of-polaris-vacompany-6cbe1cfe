...
const handleRecruitmentPracticalReady = async (body: any, token: string) => {
  ...
  if (data?.ok === false && String(data?.message || "").toLowerCase().includes("not approved") && sessionByToken?.auth_user_id) {
    ...
    if (approvedApp?.id) {
      const { data: pilotForFallback } = await supabase
        .from("pilots")
        .select("id,pid")
        .eq("user_id", sessionByToken.auth_user_id)
        .maybeSingle();
      ...
      if (pilotForFallback?.id) {
        const { data: existingPractical } = await supabase
          .from("academy_practicals")
          .select("id")
          .eq("pilot_id", pilotForFallback.id)
          .in("status", ["scheduled", "completed"])
          .ilike("notes", "Recruitment practical%")
          .order("created_at", { ascending: false })
          .limit(1)
          .maybeSingle();

        if (existingPractical?.id) {
          return buildPracticalAssignedInteraction(String(pilotForFallback.pid || "AFLV"), String(existingPractical.id));
        }

        let courseId: string | null = null;
        const { data: practicalSetting } = await supabase
          .from("site_settings")
          .select("value")
          .eq("key", "recruitment_practical_id")
          .maybeSingle();
        if (practicalSetting?.value && /^[0-9a-f-]{36}$/i.test(String(practicalSetting.value))) {
          courseId = String(practicalSetting.value);
        }
        ...
        const practicalNotes = [
          ...
          "",
          `ATYP - C172`,
          `CALLSIGN - Aeroflot ${String(pilotForFallback.pid || "AFLV").replace(/^AFLV/i, "")}CR`,
        ].join("\n");

        const { data: createdPractical, error: createError } = await supabase
          .from("academy_practicals")
          .insert({
            pilot_id: pilotForFallback.id,
            course_id: courseId,
            status: "scheduled",
        ...
        if (createError) {
          return Response.json({ type: 4, data: { content: createError.message || "Could not assign practical.", flags: 64 } });
        }
